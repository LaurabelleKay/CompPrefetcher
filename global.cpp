...
//Select the prefetcher that has the highest accuracy
int select()
{
    float max = 0;
    int pf = -1;
...
void CACHE::l2c_prefetcher_operate(uint64_t addr, uint64_t ip, uint8_t cache_hit, uint8_t type)
{
    ...
    //During trial period, call all prefetchers
    if (trial_p)
    {
        timer++;
    ...
    //After the trial period only call the selected prefetcher
    else
    {
        switch (prefetcher)
        {
...
void DISTANCE::operate(uint64_t addr, uint64_t ip, uint8_t cache_hit, uint8_t type)
{
    ...
    if (!fr)
    {
        ...
        //Prefetch distances from this address
