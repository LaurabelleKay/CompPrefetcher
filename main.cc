...
void print_roi_stats(uint32_t cpu, CACHE *cache)
{
    ...
    cout << "\"Miss " << cache->NAME << "\": " << TOTAL_MISS << ", " << endl;
}
...
int main(int argc, char **argv)
{
    ...
    if (NUM_CPUS > 1)
    {
        ...
        for (uint32_t i = 0; i < NUM_CPUS; i++)
        {
            cout << "\"Cumulative IPC\": " << (float)(ooo_cpu[i].num_retired - ooo_cpu[i].begin_sim_instr) / (current_core_cycle[i] - ooo_cpu[i].begin_sim_cycle) << "," << endl;
    ...
    for (uint32_t i = 0; i < NUM_CPUS; i++)
    {
        cout << "\"Cumulative IPC\": " << ((float)ooo_cpu[i].finish_sim_instr / ooo_cpu[i].finish_sim_cycle) << "," << endl;
